...
class NovaCompute(BaseVerifier):
    ...
    def check_no_empty_az(self) -> Result:
        ...
        target_nodes = [
            data_from_action(action, "node-name")
        ...
        affected_zones = {
            node["zone"] for node in compute_nodes if node["host"] in target_nodes
        }
        zones_after_change = {
            node["zone"]
            for node in compute_nodes
            if node["host"] not in target_nodes and is_active(node)
        }
    ...
    def verify_reboot(self) -> Result:
        ...
        return aggregate_results(self.check_no_running_vms(), self.check_no_empty_az())
